...
import com.atguigu.gmall0213.bean.dim.SkuInfo
...
object SkuInfoApp {
  def main(args: Array[String]): Unit = {
    ...
    val skuInfoDstream: DStream[SkuInfo] = objectDstream.transform { rdd =>
      ...
      if(rdd.count()>0){
        //category3
        val category3Sql  = "select id ,name   from gmall0213_base_category3" //driver  周期性执行
        ...
        //tm_name
        val tmSql = "select id ,tm_name  from gmall0213_base_trademark"
        ...
        // spu
        val spuSql = "select id ,spu_name  from gmall0213_spu_info" // spu
    ...
    skuInfoDstream.foreachRDD{rdd=>
      ...
      rdd.saveToPhoenix("GMALL0213_SKU_INFO",Seq("ID", "SPU_ID","PRICE","SKU_NAME","TM_ID","CATEGORY3_ID","CREATE_TIME","CATEGORY3_NAME","SPU_NAME","TM_NAME"  )
        ,new Configuration,Some("hadoop102,hadoop10,hadoop10:2181"))
